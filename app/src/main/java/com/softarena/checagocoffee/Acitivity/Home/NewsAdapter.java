...
public class NewsAdapter extends RecyclerView.Adapter<NewsAdapter.NotificationViewHolder> {
    private List<AllNews> allNewsList;
    Context context;
    public NewsAdapter(List<AllNews> allNewsList, Context context)
    {
    ...
    public void onBindViewHolder(@NonNull NotificationViewHolder holder, final int position) {
        holder.tv_ShopName.setText(allNewsList.get(position).getNewsName());
        Glide.with(context).load(APIClient.BASE_URL+allNewsList.get(position).getNewsImage()).into(holder.imgnews);
